...
from models.models import role, user, domain_data
...
async def test_add_domain_data(user_token):
    response = client.post("api/v1/domain/add_domain_data", headers=user_token, json={
        "main_info": {
            "domain_name": "string",
            "ls_number": 0,
            "ls": 0,
    ...
    async with async_session_maker() as session:
        ...
        assert result[0]["main_info"]["email"] == settings.USER_LOGIN
